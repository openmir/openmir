...
def run(inLiblinearFilename, timeDelta, numFrames):
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print "Usage: openmir-generate-prediction.py liblinear.txt timeDelta numFrames"
        sys.exit(1)
        
    inLiblinearFilename = sys.argv[1]
    timeDelta = int(sys.argv[2])
    numFrames = int(sys.argv[3])
    run(inLiblinearFilename, timeDelta, numFrames)
